...
# ============================================================
# client2_CIC_IOT_2023_PTFL_DESIGN_B_LOCAL_FIRST_STRICT_BARRIER.py
# ============================================================
# MATCHES Client1 PTFL logic/style:
# - SAME split rule: TEST=0.15, VAL_FROM_TRAIN=0.15/0.85
# - LOCAL-FIRST (no waiting for global to exist)
# - Safe set_shared: only if list/tuple len==2
# - Send update with ack.ok retry on round mismatch
# - STRICT BARRIER: after send at server_round=r, wait until server_round >= r+1
# - Logs: COMM_LOG, METRICS_LOG (2 rows/round), CURVE_CSV (macro-f1 only)
# - Auto binary head if num_classes==2 else softmax
# ============================================================
...
CSV_PATH  = os.environ.get(
    "CSV_PATH",
    "/Users/azizahalq/Desktop/PFTL_Multi_class/M_PFTL_codes/Multi_class_Datasets/D2_cic_iot_2023_200k_keep_selected_new.csv"
)
LABEL_COL = os.environ.get("LABEL_COL", "Label")

OUT_DIR = os.environ.get(
    "OUT_DIR",
    f"/Users/azizahalq/Desktop/PFTL_Multi_class/M_PFTL_codes/ptfl_logs/{CLIENT_ID}_v2_localfirst_strict"
)
...
# -------------------------
# Safety Switch + Adaptive Gamma (match Client1 style)
# -------------------------
ADAPT_GAMMA = True
EPS = 0.001
...
# -------------------------
# Model (match Client1: 3 conv blocks)
# -------------------------
...
# Client
# -------------------------
class PTFLClient2DesignB_LocalFirst_StrictBarrier:
    def __init__(self):
        ensure_csv_headers()
        ...
        # LOCAL-FIRST initial sync: load if exists, else start locally
    ...
    def _load_data(self):
        ...
        # Clean labels like your other clients
        y_raw = (
        ...
        print("\n=== Client2 PTFL Readiness Summary ===")
        ...
        self.class_weight = {int(c): float(w) for c, w in zip(classes_idx, cw)}

        print("\n===== SPLIT SIZES (same rule as Client1; ~70/15/15) =====")
        n = len(y)
    ...
    def train_one_epoch(self):
        ...
        # Works for binary + multiclass because metric names differ
        if self.num_classes == 2:
            train_acc = float(hist.history.get("accuracy", hist.history.get("binary_accuracy", [np.nan]))[0])
            val_acc   = float(hist.history.get("val_accuracy", hist.history.get("val_binary_accuracy", [np.nan]))[0])
        else:
    ...
    def run(self):
        for local_iter in range(NUM_ROUNDS):
            ...
            # LOCAL-FIRST: align (no waiting for global)
            ...
            # pull global (may be empty), mix + eval
            ...
            with open(CURVE_CSV, "a", newline="") as f:
                csv.writer(f).writerow([
                    ...
                    switch_on, f"{self.gamma_global:.16f}"
                ])

            # send update (retry on ack.ok==False by resyncing round)
            ...
            while True:
                ...
                if bool(getattr(ack, "ok", False)):
                    break
                # rejected (likely round mismatch): resync and retry
                self.align_to_server()
                server_round_before = int(self.current_round)

            rtt_sec = float(time.perf_counter() - send_t0)

            # STRICT BARRIER: wait until server_round >= before+1
            ...
            with open(COMM_LOG, "a", newline="") as f:
                csv.writer(f).writerow([
                    SEED, round_id, bytes_sent,
                    server_round_before, self.current_round,
                    f"{rtt_sec:.3f}", f"{waited_sec:.3f}", now_str()
                ])

            # metrics logs (two rows)
            log_stage_row(SEED, round_id, "LOCAL",
                          train_loss, train_acc, val_loss, val_acc,
                          local_acc, local_mp, local_mr, local_mf1, local_wp, local_wr, local_wf1,
                          switch_on, self.gamma_local, self.gamma_global, delta_f1, sent_type)

            log_stage_row(SEED, round_id, "GLOBAL",
                          train_loss, train_acc, val_loss, val_acc,
        ...
        macro_p, macro_r, macro_f1, _ = precision_recall_fscore_support(self.y_test, y_pred, average="macro", zero_division=0)
        w_p, w_r, w_f1, _ = precision_recall_fscore_support(self.y_test, y_pred, average="weighted", zero_division=0)
...
if __name__ == "__main__":
    ensure_csv_headers()
    PTFLClient2DesignB_LocalFirst_StrictBarrier().run()
