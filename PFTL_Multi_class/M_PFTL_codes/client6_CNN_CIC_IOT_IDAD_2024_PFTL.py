...
CLIENT_ID      = os.environ.get("CLIENT_ID", "client6")
NUM_ROUNDS     = int(os.environ.get("NUM_ROUNDS", "20"))

CSV_PATH = os.environ.get(
    "CSV_PATH",
    "/Users/azizahalq/Desktop/PFTL_project3/Datasets_processed/D6_CIC_IOT_IDAD_2024/Client6_Balanced_Final.csv"
)
LABEL_COL = os.environ.get("LABEL_COL", "Label")

OUT_DIR = os.environ.get(
    "OUT_DIR",
    f"/Users/azizahalq/Desktop/PFTL_project3/ptfl_logs/{CLIENT_ID}_intelligent_v2"
)
...
# Keep your original intelligent settings
EPS = 0.001
    ...
    if not os.path.exists(COMM_LOG):
        with open(COMM_LOG, "w", newline="") as f:
            csv.writer(f).writerow([
                ...
                "rtt_sec", "barrier_wait_sec", "local_train_time_sec", "timestamp"
            ])
...
class Client6IntelligentFixed:
    def __init__(self):
        ...
        if is_valid_shared(w):
            ...
            print(f"[{CLIENT_ID}] Initial sync: server returned empty global (server_round={self.current_round}). Starting LOCAL training first.")
    ...
    def _load_data(self):
        ...
        print("\n=== Client6 PTFL Readiness Summary ===")
    ...
    # -------------------------
    def rpc_get(self):
        return self.stub.GetSharedWeights(
    ...
    def wait_barrier_advance(self, before_round: int):
        ...
        while True:
            w, r = self.pull_global()
            print(f"[{CLIENT_ID}] Waiting at barrier... server_round={r}, target={target}")
            if int(r) >= target:
                if is_valid_shared(w):
                    self.set_shared_if_valid(w)
    ...
    def train_one_epoch(self):
        ...
        print(f"[{CLIENT_ID}] Finished local training.")
    ...
    def run(self):
        for local_iter in range(NUM_ROUNDS):
            ...
            local_train_time_sec = float(time.perf_counter() - train_t0)
            ...
            print(f"[{CLIENT_ID}] Sending update for server_round={server_round_before} ...")
            send_t0 = time.perf_counter()
            while True:
                ack = self.rpc_send(payload, num_samples=self.X_train.shape[0])
                print(f"[{CLIENT_ID}] Server ack: {ack}")

                if bool(getattr(ack, "ok", False)):
                    break

                print(f"[{CLIENT_ID}] Update rejected. Resyncing with server...")
                self.align_to_server()
                server_round_before = int(self.current_round)
            ...
            with open(COMM_LOG, "a", newline="") as f:
                csv.writer(f).writerow([
                    ...
                    f"{rtt_sec:.3f}", f"{waited_sec:.3f}", f"{local_train_time_sec:.4f}", now_str()
                ])
