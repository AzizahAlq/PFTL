...
"""
FedProto Client 3 — UNSW-NB15
================================

Fair-comparison settings preserved from the supplied PTFL Client 3:
- Seed: 190
...
- CNN:
    ...
    Dense(16)
    Dense(8) prototype embedding
    Private 34-class classifier
- Strict communication barrier

FedProto communication contract:
- SendSharedUpdate.weights contains a pickled dictionary:
    {
        "client_id": "client3",
        "round": int,
        "proto_dim": 8,
...
- GetSharedWeights.weights contains either:
    {
        global_id: {
            "semantic_label": str,
            "prototype": np.ndarray(shape=(8,), dtype=float32),
...
# Fixed experiment configuration (no environment variables required)
...
# ============================================================
# FedProto embedding visualization
# ============================================================
...
def build_fedproto_model(
    ...
    """
    Preserves the supplied Client 3 PTFL architecture.

    Important:
    The third convolution uses 64 filters, matching the supplied
    Client 3 implementation.
    """
...
class FedProtoClient3:
    ...
    def _load_local_global_mapping(self) -> None:
        ...
        print("\n" + "=" * 80)
        print("CLIENT 3 LOCAL-TO-GLOBAL MAPPING")
        print("=" * 80)
    ...
    def _load_data(self) -> None:
        if not CSV_PATH.exists():
            raise FileNotFoundError(
                f"Mapped Client 3 dataset not found:\n{CSV_PATH}"
            )
        ...
        if observed_ids != expected_ids:
            raise ValueError(
                "Client 3 labels are not contiguous.\n"
        ...
        print("\n" + "=" * 80)
        print("FEDPROTO CLIENT 3 READINESS")
        print("=" * 80)
    ...
    def save_confusion_matrices(
        ...
        plt.title("FedProto Client 3 — Raw Confusion Matrix")
        plt.colorbar()
        ...
        plt.title(
            "FedProto Client 3 — Row-Normalized Confusion Matrix"
        )
        plt.colorbar()
        plt.xticks(ticks, class_names, rotation=90, fontsize=7)
        plt.yticks(ticks, class_names, fontsize=7)
    ...
    def save_convergence_curve(
        ...
        plt.ylabel("Score")
        plt.title("FedProto Client 3 Convergence")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig(
    ...
    def visualize_prototype_embedding(
        ...
        # ----------------------------------------------------
        # Stratified subsampling, same setup as Client 1.
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # predict() already returns the 8-D prototype_embedding.
        # ----------------------------------------------------
        ...
        label_names = np.asarray(
            [
                normalize_label_name(
                    self.local_to_semantic[int(label_id)]
                )
        ...
        # Fixed semantic colors and legend order are used below.
        # This makes the Standalone and FedProto figures directly
        # comparable at the visualization level.
        ...
        plt.title(
            ...
            f"{CLIENT_ID} | {self.num_classes} Classes | Train Seed={SEED} | t-SNE Seed={TSNE_SEED}"
        )
        ...
        if UMAP_AVAILABLE:
            ...
            umap_2d = umap_model.fit_transform(
                embeddings
            )
            ...
            plt.title(
                ...
                f"{CLIENT_ID} | {self.num_classes} Classes | Train Seed={SEED} | UMAP Seed={UMAP_SEED}"
            )
        ...
        print("\n" + "=" * 80)
        print(
            f"[{CLIENT_ID}] FEDPROTO EMBEDDING VISUALIZATION COMPLETE"
        )
        print("=" * 80)
    ...
    def finalize_experiment(self) -> None:
        ...
        # ----------------------------------------------------
        # Preserve final metrics from all seeds.
        #
        # Behavior:
        # - Create the CSV with a header if it does not exist.
        # - Preserve all previously completed seeds.
        # - Replace only the current seed/client/dataset row when
        #   the same experiment is rerun.
        # - Write through a temporary file to avoid corruption.
        # ----------------------------------------------------
        ...
        if FINAL_METRICS_CSV.exists():
            try:
                existing_metrics = pd.read_csv(FINAL_METRICS_CSV)

                # Preserve old values and add any missing columns.
                for column in expected_columns:
                    if column not in existing_metrics.columns:
                        existing_metrics[column] = np.nan

                seed_values = pd.to_numeric(
                    existing_metrics["seed"],
                    errors="coerce",
                )

                same_run_mask = (
                    seed_values.eq(SEED)
                    & existing_metrics["client_id"].astype(str).eq(CLIENT_ID)
                    & existing_metrics["dataset"].astype(str).eq(
                        "UNSW-NB15"
                    )
                )

                # Replace only an existing row for the same run.
                existing_metrics = existing_metrics.loc[
                    ~same_run_mask
                ].copy()

                combined_metrics = pd.concat(
                    [existing_metrics, final_row],
                    ignore_index=True,
                    sort=False,
                )

            except pd.errors.EmptyDataError:
                combined_metrics = final_row.copy()
            except Exception as exc:
                raise RuntimeError(
                    f"Could not safely update final metrics file "
                    f"{FINAL_METRICS_CSV}: {exc}"
                ) from exc
        else:
            combined_metrics = final_row.copy()

        # Keep the requested columns first and preserve any
        # extra columns already present in the CSV.
        extra_columns = [
            column
        ...
        # Sort rows by seed for easier analysis.
        combined_metrics["_seed_sort"] = pd.to_numeric(
            combined_metrics["seed"],
            errors="coerce",
        )
        combined_metrics = (
            combined_metrics
            .sort_values(
                by=["_seed_sort", "client_id", "dataset"],
                kind="stable",
                na_position="last",
            )
        ...
        temporary_metrics_csv = FINAL_METRICS_CSV.with_suffix(
            FINAL_METRICS_CSV.suffix + ".tmp"
        )
        ...
        print(
            f"[{CLIENT_ID}] Final metrics preserved for "
            f"{len(combined_metrics)} seed run(s): "
            f"{FINAL_METRICS_CSV}"
        )
        ...
        # ----------------------------------------------------
        # Final FedProto embedding visualization
        # ----------------------------------------------------
