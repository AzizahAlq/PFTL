...
"""
FedProto Client 4 — CIC-IDS-2017
================================

Fair-comparison settings preserved from the supplied PTFL Client 4:
- Seed: 45
...
CSV_PATH = (
    ...
    / "D4_CIC-IDS-2017_FedProto_mapped.csv"
)
...
def build_fedproto_model(
    ...
    """
    Preserves the supplied Client 4 PTFL architecture.

    Important:
    The third convolution uses 64 filters, matching the supplied
    Client 4 implementation.
    """
...
class FedProtoClient4:
    ...
    def _load_local_global_mapping(self) -> None:
        ...
        print("\n" + "=" * 80)
        print("CLIENT 4 LOCAL-TO-GLOBAL MAPPING")
        print("=" * 80)
    ...
    def _load_data(self) -> None:
        if not CSV_PATH.exists():
            raise FileNotFoundError(
                f"Mapped Client 4 dataset not found:\n{CSV_PATH}"
            )
        ...
        if observed_ids != expected_ids:
            raise ValueError(
                "Client 4 labels are not contiguous.\n"
        ...
        print("\n" + "=" * 80)
        print("FEDPROTO CLIENT 4 READINESS")
        print("=" * 80)
    ...
    def save_confusion_matrices(
        ...
        plt.title("FedProto Client 4 — Raw Confusion Matrix")
        plt.colorbar()
        ...
        plt.title(
            "FedProto Client 4 — Row-Normalized Confusion Matrix"
        )
        plt.colorbar()
        plt.xticks(ticks, class_names, rotation=90, fontsize=7)
        plt.yticks(ticks, class_names, fontsize=7)
    ...
    def save_convergence_curve(
        ...
        plt.ylabel("Score")
        plt.title("FedProto Client 4 Convergence")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig(
    ...
    def finalize_experiment(self) -> None:
        ...
        final_row = pd.DataFrame([{
            ...
            **final_metrics,
            ...
            "best_validation_macro_f1": convergence_summary["best_validation_macro_f1"],
            "convergence_round": convergence_summary["convergence_round"],
            "convergence_time_sec": convergence_summary["convergence_time_sec"],
            "total_experiment_time_sec": total_time,
            "average_round_time_sec": average_round_time,
        }])

        expected_columns=[
            "seed","client_id","dataset","accuracy","macro_precision",
            "macro_recall","macro_f1","weighted_precision","weighted_recall",
            "weighted_f1","best_round","best_validation_macro_f1",
        ...
        if FINAL_METRICS_CSV.exists():
            try:
                ...
                for c in expected_columns:
                    if c not in existing_metrics.columns:
                        existing_metrics[c]=np.nan

                mask=(
                    pd.to_numeric(existing_metrics["seed"],errors="coerce").eq(SEED)
                    & existing_metrics["client_id"].astype(str).eq(CLIENT_ID)
                    & existing_metrics["dataset"].astype(str).eq("CIC-IDS-2017")
                )
                existing_metrics=existing_metrics.loc[~mask].copy()
                combined_metrics=pd.concat([existing_metrics,final_row],ignore_index=True,sort=False)
            except pd.errors.EmptyDataError:
                combined_metrics=final_row.copy()
        else:
            combined_metrics=final_row.copy()

        extras=[c for c in combined_metrics.columns if c not in expected_columns]
        combined_metrics=combined_metrics[expected_columns+extras]
        combined_metrics["_seed_sort"]=pd.to_numeric(combined_metrics["seed"],errors="coerce")
        combined_metrics=(combined_metrics.sort_values(["_seed_sort","client_id","dataset"],kind="stable")
                          .drop(columns="_seed_sort").reset_index(drop=True))
        tmp=FINAL_METRICS_CSV.with_suffix(FINAL_METRICS_CSV.suffix+".tmp")
        combined_metrics.to_csv(tmp,index=False)
        tmp.replace(FINAL_METRICS_CSV)
        print(f"[{CLIENT_ID}] Preserved {len(combined_metrics)} seed results.")
