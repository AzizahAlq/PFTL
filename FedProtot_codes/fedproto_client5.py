...
"""
FedProto Client 5 — CIC-BCCC-NRC-2024
================================

Fair-comparison settings preserved from the supplied PTFL Client 5:
- Seed: 45
...
IMPUTER_PATH = OUT_DIR / f"{CLIENT_ID}_median_imputer.pkl"
...
def build_fedproto_model(
    ...
    """
    Preserves the supplied Client 5 PTFL architecture.

    Important:
    The third convolution uses 64 filters, matching the supplied
    Client 5 implementation.
    """
...
class FedProtoClient5:
    ...
    def _load_local_global_mapping(self) -> None:
        ...
        print("\n" + "=" * 80)
        print("CLIENT 5 LOCAL-TO-GLOBAL MAPPING")
        print("=" * 80)
    ...
    def _load_data(self) -> None:
        if not CSV_PATH.exists():
            raise FileNotFoundError(
                f"Mapped Client 5 dataset not found:\n{CSV_PATH}"
            )
        ...
        if observed_ids != expected_ids:
            raise ValueError(
                "Client 5 labels are not contiguous.\n"
        ...
        # Match the original Client 5 preprocessing:
        ...
        print("\n" + "=" * 80)
        print("FEDPROTO CLIENT 5 READINESS")
        print("=" * 80)
    ...
    def save_confusion_matrices(
        ...
        plt.title("FedProto Client 5 — Raw Confusion Matrix")
        plt.colorbar()
        ...
        plt.title(
            "FedProto Client 5 — Row-Normalized Confusion Matrix"
        )
        plt.colorbar()
        plt.xticks(ticks, class_names, rotation=90, fontsize=7)
        plt.yticks(ticks, class_names, fontsize=7)
    ...
    def save_convergence_curve(
        ...
        plt.ylabel("Score")
        plt.title("FedProto Client 5 Convergence")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig(
