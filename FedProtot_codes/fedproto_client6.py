...
"""
FedProto Client 6 — CIC-IoT-IDAD-2024
================================

Fair-comparison settings preserved from the supplied PTFL Client 6:
- Seed: 45
...
CSV_PATH = (
    ...
    / "D6_CIC-IoT-IDAD-2024_FedProto_mapped.csv"
)
...
def build_fedproto_model(
    ...
    """
    Preserves the supplied Client 6 PTFL architecture.
...
class FedProtoClient6:
    ...
    def _load_local_global_mapping(self) -> None:
        ...
        print("\n" + "=" * 80)
        print("CLIENT 6 LOCAL-TO-GLOBAL MAPPING")
        print("=" * 80)
    ...
    def _load_data(self) -> None:
        if not CSV_PATH.exists():
            raise FileNotFoundError(
                f"Mapped Client 6 dataset not found:\n{CSV_PATH}"
            )
        ...
        if observed_ids != expected_ids:
            raise ValueError(
                "Client 6 labels are not contiguous.\n"
        ...
        # Match the original Client 6 preprocessing:
        ...
        print("\n" + "=" * 80)
        print("FEDPROTO CLIENT 6 READINESS")
        print("=" * 80)
    ...
    def save_confusion_matrices(
        ...
        plt.title("FedProto Client 6 — Raw Confusion Matrix")
        plt.colorbar()
        ...
        plt.title(
            "FedProto Client 6 — Row-Normalized Confusion Matrix"
        )
        plt.colorbar()
        plt.xticks(ticks, class_names, rotation=90, fontsize=7)
        plt.yticks(ticks, class_names, fontsize=7)
    ...
    def save_convergence_curve(
        ...
        plt.ylabel("Score")
        plt.title("FedProto Client 6 Convergence")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig(
