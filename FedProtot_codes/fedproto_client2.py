...
"""
FedProto Client 2 — CIC-IoT-2023
================================

Fair-comparison settings preserved from the supplied PTFL Client 2:
- Seed: 45
...
# Everything needed to run Client 2 is defined in this file.
# No terminal environment variables are required.
# ============================================================
...
def build_fedproto_model(
    ...
    """
    Preserves the supplied Client 2 PTFL architecture.
    ...
    x = layers.Conv1D(
        64, 3, padding="valid", activation="relu", name="private_conv3"
    )(x)
...
class FedProtoClient2:
    ...
    def _load_local_global_mapping(self) -> None:
        ...
        print("\n" + "=" * 80)
        print("CLIENT 2 LOCAL-TO-GLOBAL MAPPING")
        print("=" * 80)
    ...
    def _load_data(self) -> None:
        if not CSV_PATH.exists():
            raise FileNotFoundError(
                f"Mapped Client 2 dataset not found:\n{CSV_PATH}"
            )
        ...
        if observed_ids != expected_ids:
            raise ValueError(
                "Client 2 labels are not contiguous.\n"
        ...
        print("\n" + "=" * 80)
        print("FEDPROTO CLIENT 2 READINESS")
        print("=" * 80)
    ...
    def save_confusion_matrices(
        ...
        plt.title("FedProto Client 2 — Raw Confusion Matrix")
        plt.colorbar()
        ...
        plt.title(
            "FedProto Client 2 — Row-Normalized Confusion Matrix"
        )
        plt.colorbar()
        plt.xticks(ticks, class_names, rotation=90, fontsize=7)
        plt.yticks(ticks, class_names, fontsize=7)
    ...
    def save_convergence_curve(
        ...
        plt.ylabel("Score")
        plt.title("FedProto Client 2 Convergence")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig(
    ...
    def finalize_experiment(self) -> None:
        ...
        #
        # Behavior:
        # - If the CSV does not exist, create it with a header.
        # - If the CSV exists, preserve all previous seeds.
        # - If the same seed/client/dataset is rerun, replace only
        ...
        if FINAL_METRICS_CSV.exists():
            try:
                ...
                # Add any missing expected columns without deleting
                # previously stored values.
                for column in expected_columns:
                    if column not in existing_metrics.columns:
                        existing_metrics[column] = np.nan

                # Remove only the row belonging to this exact
                # seed/client/dataset combination.
        ...
        # Keep the requested columns first while preserving any
        # additional columns that may already exist.
        extra_columns = [
            column
        ...
        # Sort for easier comparison across seeds.
