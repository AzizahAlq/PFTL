...
import os, time, csv, pickle, random, json
...
CSV_PATH  = os.environ.get(
    "CSV_PATH",
    "/nfs/aalqahtani/PFTL_Multi_class/M_PFTL_codes/Multi_class_Datasets/D1_CIC-ToN-IoT_new.csv"
)
LABEL_COL = os.environ.get("LABEL_COL", "Attack")

OUT_DIR = os.environ.get(
    "OUT_DIR",
    "/nfs/aalqahtani/PFTL_Multi_class/M_PFTL_codes/ptfl_logs/client1_ton_iot_v2"
)
...
FINAL_METRICS_CSV = os.path.join(OUT_DIR, f"{CLIENT_ID}_final_metrics.csv")
CONVERGENCE_SUMMARY_JSON = os.path.join(OUT_DIR, f"{CLIENT_ID}_convergence_summary.json")
CONFUSION_MATRIX_CSV = os.path.join(OUT_DIR, f"{CLIENT_ID}_confusion_matrix.csv")
CONFUSION_MATRIX_NORMALIZED_CSV = os.path.join(
    OUT_DIR, f"{CLIENT_ID}_confusion_matrix_normalized.csv"
)
CLASSIFICATION_REPORT_CSV = os.path.join(
    OUT_DIR, f"{CLIENT_ID}_classification_report.csv"
)
MODEL_PATH = os.path.join(OUT_DIR, f"{CLIENT_ID}_ptfl_final_model.keras")
SCALER_PATH = os.path.join(OUT_DIR, f"{CLIENT_ID}_standard_scaler.pkl")
...
class PTFLClientDesignB_StrictBarrier:
    ...
    # -------------------------
    # Comparable reporting helpers
    # -------------------------
    def determine_convergence(self):
        ...
        values = np.asarray(
            [row["global_macro_f1"] for row in self.round_history],
            dtype=np.float64,
        )

        best_index = int(np.argmax(values))
        final_value = float(values[-1])
        threshold = float(CONVERGENCE_RATIO * final_value)

        convergence_index = None
        for start in range(len(values)):
            end = start + CONVERGENCE_PATIENCE
            if end > len(values):
                break
            if np.all(values[start:end] >= threshold):
                convergence_index = start
                break
        ...
        return {
            ...
            "best_round": int(self.round_history[best_index]["round"]),
            "best_validation_macro_f1": float(values[best_index]),
            "final_validation_macro_f1": final_value,
            "convergence_threshold_macro_f1": threshold,
            "convergence_round": (
                None if convergence_index is None
                else int(self.round_history[convergence_index]["round"])
            ),
            "convergence_time_sec": (
                None if convergence_index is None
                else float(
                    self.round_history[convergence_index]["cumulative_time_sec"]
                )
            ),
        }

    def save_final_outputs(self, y_pred):
        ...
        convergence = self.determine_convergence()
        ...
        final_row = {
            ...
            "best_round": convergence["best_round"],
            "best_validation_macro_f1": (
                convergence["best_validation_macro_f1"]
            ),
            "convergence_round": convergence["convergence_round"],
            "convergence_time_sec": convergence["convergence_time_sec"],
            ...
            "shared_dimension": int(SHARED_DIM),
        }
        ...
        with open(CONVERGENCE_SUMMARY_JSON, "w", encoding="utf-8") as f:
    ...
    def run(self):
        for local_iter in range(NUM_ROUNDS):
            ...
            # comparable timing and communication log
            round_total_time = time.perf_counter() - round_start
            cumulative_time = (
                time.perf_counter() - self.experiment_start_time
            )

            # shared_dense has kernel + bias. This counts numeric values,
            # while payload_bytes_sent measures the actual serialized payload.
            shared_parameter_values_sent = int(
                sum(np.asarray(array).size for array in weights_to_send)
            )
            ...
            with open(COMM_LOG, "a", newline="") as f:
                csv.writer(f).writerow([
                    ...
                    shared_parameter_values_sent,
                    f"{float(rtt):.6f}",
                    f"{float(barrier_wait):.6f}",
                    f"{float(round_total_time):.6f}",
                    f"{float(cumulative_time):.6f}",
                    now_str()
                ])
            ...
            self.round_history.append({
                "round": int(round_id),
                "local_macro_f1": float(local_mf1),
                "global_macro_f1": float(mixed_mf1),
                "switch_on": int(switch_on),
